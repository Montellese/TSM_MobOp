...
public class Font
{
  ...
  {
    this.texture = new Texture(gl, 256, 256, TextureFilter.Nearest,
  ...
  public Font(GL10 gl, AssetManager assets, String file, int size,
      FontStyle style)
  {
    this.texture = new Texture(gl, 256, 256, TextureFilter.Nearest,
        TextureFilter.Nearest, TextureWrap.ClampToEdge, TextureWrap.ClampToEdge);
  ...
  public Object getGlyphBitmap(char character)
  {
    ...
    Bitmap bitmap = Bitmap.createBitmap(rect.width() == 0 ? 1
        : rect.width() + 5, getLineHeight(), Bitmap.Config.ARGB_8888);
  ...
  {
    return (int) Math
        .ceil(Math.abs(metrics.ascent) + Math.abs(metrics.descent));
  }
  ...
  private Glyph createGlyph(char character)
  {
    ...
    Glyph glyph = new Glyph(getGlyphAdvance(character), (int) rect.width,
        (int) rect.height, glyphX / 256.0f, glyphY / 256.0f,
        rect.width / 256.0f, rect.height / 256.0f);
    glyphX += rect.width;
    return glyph;
  }
  ...
  private class Glyph
  {
    ...
    public Glyph(int advance, int width, int height, float u, float v,
